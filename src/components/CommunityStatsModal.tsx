 ...
 * בקיבוץ עלומים בלייב מ-Firestore.
 */

import React, { useState, useEffect } from 'react';
import { Photo, VoteResult } from '../types';
import { getCommunityScores, fetchCommunityVotes } from '../utils/firebase';
...
export const CommunityStatsModal: React.FC<CommunityStatsModalProps> = ({
  ...
  const [votesList, setVotesList] = useState<VoteResult[]>([]);
  const [loading, setLoading] = useState<boolean>(true);

  // טעינת ההצבעות מ-Firestore בעת פתיחת המודל
  useEffect(() => {
    let isMounted = true;
    
    fetchCommunityVotes().then((fetchedVotes) => {
      if (isMounted) {
        setVotesList(fetchedVotes);
        setLoading(false);
      }
    });
  ...
  const communityStats = getCommunityScores(photos, votesList);
  const { modals } = SITE_CONFIG;

  // סך כל ההצבעות שנרשמו ב-Firestore
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn">
      <div className="bg-white border border-sky-100 rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative text-slate-800 space-y-6 max-h-[90vh] flex flex-col">
        ...
        {/* תצוגת טעינה או רשימת התמונות */}
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center space-y-3 py-12">
            <Loader2 className="w-8 h-8 text-[#2c7a66] animate-spin" />
            <p className="text-sm font-semibold text-slate-600">טוען תוצאות מ-Firestore...</p>
          </div>
        ) : (
